defmodule Bsky.Auth do
  ...
  @spec add_token_to_headers(String.t()) :: [{atom(), String.t()}]
  defp add_token_to_headers(token), do: [{:Authorization, "Bearer #{token}"} | @headers]
  ...
  @spec get_jwt(HTTPoison.Response.t()) :: Either.t()
  defp get_jwt(%HTTPoison.Response{} = response) do
    response.body
    |> Jason.decode()
    |> Either.to_either()
  ...
  defp create_session(body) do
    Http.post(@create_session_url, body, @headers)
  end
